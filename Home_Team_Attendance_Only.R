...
ags <- merge(as, games2, by = c("team", "year", "week"))
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("year_range", "Select Year Range",
                  min = min(ags$year), max = max(ags$year),
                  value = c(min(ags$year), max(ags$year)), step = 1),
      selectInput("stat_input", "Select Stat", choices = colnames(ags)[-c(1:3)])
    ),
    mainPanel(
      tabsetPanel(
        type = "tabs",
